...
for c_i, cls_type in enumerate(class_type_list):
    for n_classes in class_nums:
        ...
        dat_arg = make_run_args_nengo(dat, desc["dims"], desc["n_classes"], int(desc["t_len"]/dt))

        for e_i in range(exp_iter):
            aug_res = aug(dat, mk_res[1], 1, add_rand_noise, {})
            test_arg = make_run_args_nengo(np.array(aug_res), desc["dims"], desc["n_classes"], int(desc["t_len"]/dt))
